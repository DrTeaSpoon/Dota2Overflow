...
function UpdateAbility()
{
	...
	if (SameTeam( m_QueryUnit )){
		var abilityButton = $( "#AbilityButton" );
		if (m_Ability == -1){
		var abilityName = "invoker_empty1";
		}
		else
		{
	...
	else
	{
		...
		$.GetContextPanel().SetHasClass( "insufficient_mana", false );
		$.GetContextPanel().SetHasClass( "auto_cast_enabled", false);
		$.GetContextPanel().SetHasClass( "toggle_enabled", false );
		$.GetContextPanel().SetHasClass( "is_active", false );
		abilityButton.enabled = ( isCastable || m_bInLevelUp );
		$( "#HotkeyText" ).text = hotkey;
		$( "#AbilityImage" ).abilityname = abilityName;
		$( "#AbilityImage" ).contextEntityIndex = m_Ability;
