...
function UpdateBuff( buffPanel, queryUnit, buffSerial )
{
	...
	if (Buffs.GetDuration(queryUnit, buffSerial) > 0 ){
		buffPanel.SetHasClass( "has_duration", true );
		var durLength = Buffs.GetDuration( queryUnit, buffSerial ); 
		var durRemaining = Buffs.GetElapsedTime( queryUnit, buffSerial  );
		var durPercent = Math.ceil( 100 * durRemaining / durLength );
		var durPanel = buffPanel.FindChildInLayoutFile( "DurationOverlay" );
		durPanel.style.width = durPercent+"%";
	}
...
function UpdateBuffs()
{
	...
	for ( var i = 0; i < nBuffs; ++i )
	{
		...
		if ( nUsedPanels >= m_BuffPanels.length )
		{
			...
			buffPanel.BLoadLayout( "file://{resources}/layout/custom_game/OverflowHud/buff_list_buff.xml", false, false );
