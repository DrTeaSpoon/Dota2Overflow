if item_mjollnir_ovf_mod == nil then
	item_mjollnir_ovf_mod = class({})
end

function item_mjollnir_ovf_mod:DeclareFunctions()
	local funcs = {-
...
function item_mjollnir_ovf_mod:OnCreated()
	if IsServer() then
		self:StartIntervalThink( 1 )
	end
end

function item_mjollnir_ovf_mod:IsHidden()
	return true
end

function item_mjollnir_ovf_mod:GetAttributes() 
	return MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE + MODIFIER_ATTRIBUTE_MULTIPLE
end

function item_mjollnir_ovf_mod:GetModifierPreAttack_BonusDamage()
...
function item_mjollnir_ovf_mod:OnIntervalThink()
	if IsServer() then
