if item_multicast_spell_modifier == nil then
	item_multicast_spell_modifier = class({})
end

function item_multicast_spell_modifier:DeclareFunctions()
	local funcs = {
...
function item_multicast_spell_modifier:GetModifierBonusStats_Agility()
	local hMulticast = self:GetAbility()
	return hMulticast:GetSpecialValueFor( "all_stats" ) 
end
function item_multicast_spell_modifier:GetModifierBonusStats_Intellect()
	local hMulticast = self:GetAbility()
	return hMulticast:GetSpecialValueFor( "all_stats" ) 
end
function item_multicast_spell_modifier:GetModifierBonusStats_Strength()
	local hMulticast = self:GetAbility()
	return hMulticast:GetSpecialValueFor( "all_stats" ) 
end
  
  
function item_multicast_spell_modifier:OnAbilityExecuted( params )
	if IsServer() then
			...
			if hAbility ~= nil and ( not hAbility:IsItem() or self:GetParent():HasScepter() ) and bit.band(DOTA_ABILITY_BEHAVIOR_AUTOCAST , hAbility:GetBehavior() ) ~= DOTA_ABILITY_BEHAVIOR_AUTOCAST and bit.band(DOTA_ABILITY_BEHAVIOR_TOGGLE , hAbility:GetBehavior() ) ~= DOTA_ABILITY_BEHAVIOR_TOGGLE then
				...
				while self:GetStackCount() > 0 do
                  ...
                  if multAb then
					...
					multUnit:AddNewModifier(multUnit, self:GetAbility() , "item_multicast_dummy_kill_modifier", {})
...
function item_multicast_spell_modifier:OnCreated( kv )	
	self.ChannelEnabled = true
end

function item_multicast_spell_modifier:IsHidden()
	return true
end

function item_multicast_spell_modifier:MulticastEffect( nMult )
	if IsServer() then
		local hCaster = self:GetParent()
...
function item_multicast_spell_modifier:IsItemBanned(hAbility)
	if IsServer() then
