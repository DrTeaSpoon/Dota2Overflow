if arc_lightning_lua_modifier == nil then
	arc_lightning_lua_modifier = class({})
end

function arc_lightning_lua_modifier:OnCreated( kv )	
	if IsServer() then
...
function arc_lightning_lua_modifier:IsHidden()
	return true
end


function arc_lightning_lua_modifier:OnDestroy()
	if IsServer() then
...
function arc_lightning_lua_modifier:DamageParent()
	if IsServer() then
		local damage = {
...
function arc_lightning_lua_modifier:HasProtection(hTarget)
	if IsServer() then
		local hAbility = self:GetAbility()
...
function arc_lightning_lua_modifier:IsPurgable()
	return false
end

function arc_lightning_lua_modifier:GetAttributes() 
