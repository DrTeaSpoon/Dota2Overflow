...
LinkLuaModifier( "aa_ice_path_stun", "lua_abilities/AA/ice_path_stun.lua", LUA_MODIFIER_MOTION_NONE )
function aa_ice_path:GetCastAnimation()
	return ACT_DOTA_ATTACK
end

function aa_ice_path:GetPlaybackRateOverride()
	return 0.5
end

function aa_ice_path:OnSpellStart()
	local hCaster		= self:GetCaster()
	local pathLength	= self:GetCastRange( self:GetCursorPosition(), nil)
	...
	EmitSoundOnLocationWithCaster(endPosSnd, "Hero_Jakiro.IcePath", hCaster )
	-- Create ice_path
...
function aa_ice_path:OnProjectileHit( hTarget, vLocation )
	local hCaster = self:GetCaster()
	local stun_remaining = self.ice_path_stunEnd - GameRules:GetGameTime()
	if hTarget then
	hTarget:AddNewModifier( hCaster, self, "aa_ice_path_stun", { duration = stun_remaining } )
	end
end

--function aa_ice_path:ApplyDummyModifier()
--	local hCaster		= self:GetCaster()
...
--function aa_ice_path:CheckIcePath()
--	local hCaster		= self:GetCaster()
--	local target		= event.target
...
--end
--
--function aa_ice_path:DistancePointSegment( p, v, w )
--	local l = w - v
--	local l2 = l:Dot( l )
--	t = ( p - v ):Dot( w - v ) / l2
--	if t < 0.0 then
