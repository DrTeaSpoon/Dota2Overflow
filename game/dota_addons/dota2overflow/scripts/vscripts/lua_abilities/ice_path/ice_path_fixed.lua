...
function ice_path:OnSpellStart()
	...
	local pathLength	= self:GetSpecialValueFor( "range" )
	...
	local projectilePos = startPos
		...
		ProjectileManager:CreateLinearProjectile( {
			...
			vStart = self.ice_path_startPos,
			vEnd = self.ice_path_endPos,
			fStart = GameRules:GetGameTime(),
		} )
...
function ice_path:OnProjectileThink_ExtraData(vLocation, hData)
	local hCaster = self:GetCaster()
	local units = FindUnitsInLine( hCaster:GetTeamNumber(), hData.vStart, hData.vEnd, nil, hData.fStartRadius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, FIND_ANY_ORDER  )
