if corrupted_blink == nil then
	corrupted_blink = class({})
end

LinkLuaModifier( "corrupted_blink_effect_modifier", "lua_abilities/corrupted_blink/effect_modifier.lua", LUA_MODIFIER_MOTION_NONE )

function corrupted_blink:GetBehavior() 
	local behav = DOTA_ABILITY_BEHAVIOR_POINT + DOTA_ABILITY_BEHAVIOR_UNIT_TARGET + DOTA_ABILITY_BEHAVIOR_ROOT_DISABLES
	return behav
end

function corrupted_blink:GetManaCost()
	return self.BaseClass.GetManaCost( self, 1 )
end

function corrupted_blink:GetCooldown( nLevel )
...
function corrupted_blink:GetCastAnimation( )
	local anim = ACT_DOTA_FLAIL
	return anim
end

function corrupted_blink:OnSpellStart()
	...
	if hTarget then
		if hCaster == hTarget then
			...
			local hModifier = hCaster:FindModifierByNameAndCaster("corrupted_blink_effect_modifier", hCaster) --Check if we have someone selected
			if hModifier then
				hOld = EntIndexToHScript(hModifier:GetStackCount()) --Find the target with the ent index
				if hOld:FindModifierByNameAndCaster("corrupted_blink_effect_modifier", hCaster) then --Check if the target is not purged.
					hCaster:RemoveModifierByNameAndCaster("corrupted_blink_effect_modifier", hCaster)
					hOld:RemoveModifierByNameAndCaster("corrupted_blink_effect_modifier", hCaster)
				end
			end
			hTarget:AddNewModifier( hCaster, self, "corrupted_blink_effect_modifier", { duration = self:GetSpecialValueFor( "help_duration" ) } ) --lets add modifier to target
			hCaster:AddNewModifier( hCaster, self, "corrupted_blink_effect_modifier", { duration = self:GetSpecialValueFor( "help_duration" ) } ) --lets add modifier to caster
			local hModifier = hCaster:FindModifierByNameAndCaster("corrupted_blink_effect_modifier", hCaster) --find that modifier (they really should fix this by returning handle when adding new modifier.
		...
		if hModifier then
			...
			if hTarget:FindModifierByNameAndCaster("corrupted_blink_effect_modifier", hCaster) then --Check if the target is not purged.
...
function corrupted_blink:Blink(hTarget, vPoint, nMaxBlink, nClamp)
...
function corrupted_blink:CastFilterResultTarget( hTarget ) -- hTarget is the targeted NPC.
	return self:CCastFilter( hTarget, false )
end

function corrupted_blink:GetCustomCastErrorTarget( hTarget) -- hTarget is the targeted NPC. 
	return self:CCastFilter( hTarget, true )
end

function corrupted_blink:CCastFilter( hTarget, bError )
