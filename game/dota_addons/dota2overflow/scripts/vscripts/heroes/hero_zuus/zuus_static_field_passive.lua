...
function zuus_static_field_lua_passive:OnAbilityExecuted()
	if IsServer() then
		local hAbility = self:GetAbility()
		if hAbility:IsItem() then return end
		local iAoE = hAbility:GetSpecialValueFor( "radius" )
		local iDamageHpPct = hAbility:GetSpecialValueFor( "damage_health_pct" ) / 100
